...
sub update ($m, $task_id, $fields_to_update) {
  ...
  $m->app->db->update($m->t_tasks, $upd_task_fields, {id => $task_id}) if $upd_task_fields;

  my $task                = $m->lookup($task_id);
  my $need_update_by_type = $m->_check_for_update_by_type($task, $fields_to_update);

  $m->_process_update_for_type($task, $fields_to_update) if $need_update_by_type;
...
sub _check_for_update_by_type ($m, $task, $fields_to_update) {
  my $need_update;
  $need_update = $m->app->model('challenge-task-days')->check_for_update($fields_to_update) if $task->{type} eq 'days';

  return $need_update;
}

sub _process_update_for_type ($m, $task, $fields_to_update) {
  $m->app->model('challenge-task-days')->process_update($task->{id}, $fields_to_update) if $task->{type} eq 'days';
}

sub _process_list_for_type ($m, $task) {
  $task->{days} = $m->app->model('challenge-task-days')->list($task->{id}) if $task->{type} eq 'days';
}
