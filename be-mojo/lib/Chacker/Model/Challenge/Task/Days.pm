...
sub list ($m, $task_id) {
  $m->app->db->select($m->t_days, $m->f_days, {task_id => $task_id})->hashes->to_array;
}
...
sub process_update ($m, $task_id, $fields_to_update) {
  ...
  if (scalar(@$days2add) >= 1) {
    $m->app->db->insert($m->t_days, {task_id => $task_id, day => $_}) for @$days2add;
  }

  if (scalar(@$days2remove) >= 1) {
    $m->app->db->delete($m->t_days, {task_id => $task_id, day => $_}) for @$days2remove;
  }
